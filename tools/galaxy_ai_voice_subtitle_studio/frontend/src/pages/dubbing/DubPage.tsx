...
export function DubPage() {
  ...
  const handleGenerateDone = (task: TaskState) => {
    if (task.status === 'done' && task.result) {
      setGenerateResult(task.result as unknown as GenerateResultPayload)
    }
  }
  ...
  const handleTranscribeDone = (task: TaskState) => {
    if (task.status === 'done') {
        ...
        <section className="section-card">
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 12, alignItems: 'center' }}>
            <TaskButton
              label="Tạo voice"
              variant="accent"
              onStart={async () => (await startGenerate({ text: script, ...commonVoiceOptions() })).task_id}
        ...
        <section className="section-card">
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
            <TaskButton
              ...
              onFinish={handleTranscribeDone}
            />
          </div>
        </section>
      </div>

      {draftTaskId && <DraftEditor taskId={draftTaskId} />}
    </div>
  )
}
