...
describe('AudioPostPanel', () => {
  ...
  it('loads a bounded waveform and sends the shared post chain', async () => {
    ...
    vi.mocked(exportAudio).mockResolvedValue({
    ...
    render(<AudioPostPanel projectId="project-1" workspace="studio" projectDir="D:/project" title="Final" sources={[{ source_id: 'voice', label: 'Voice', path: 'D:/project/voice.wav', role: 'voice', preview_url: '/voice.wav' }]} />)
    ...
    expect(vi.mocked(exportAudio).mock.calls[0][0]).toMatchObject({
      project_id: 'project-1', workspace: 'studio', formats: ['wav', 'mp3'],
